# -*- coding: utf-8 -*-
"""Stage the gb fragments for topicbuild: strip the gb- prefix (and the -en
suffix on the English side) and insert the site's article-figure markup at
the position the manifest prescribes.

Like lv / brt / pel / ec, every gb figure carries placement.after_h4 -- the
...
PLACEMENT IS THE AUTHORITY.  All eight gb figures have a single-article
used_by that agrees with placement, so eight figures make eight insertions,
one per article in A1 / B1 / B2 / B3 / B4 / C2 / C4 / D1.

Figures are inserted HERE, into the staged copy the builder reads, so the
source fragments under /home/claude/gbm/body stay figure-free and remain the
...
height; the gb SVGs carry integer heights, but the pel-style round-half-up is
kept so a fractional one would not break the build.

Set GB_STAGE to match gbm.py's _STAGE.
"""
import decimal
import glob
import json
...
GB = "/home/claude/gbm"
STAGE = os.environ.get("GB_STAGE", "./staging-gb")
...
def svg_height(name):
    ...
    s = open(GB + "/figs/" + name, encoding="utf-8").read()
...
def insert_after_section(text, n, block):
    ...
    <h4> #(n+1).  Every gb placement has a following section (asserted)."""
...
def stage():
    ...
    manifest = json.load(open(GB + "/figs/manifest.json", encoding="utf-8"))
    per = {}
    for fig in manifest:
        art = fig["placement"]["article"]
        assert art in fig["used_by"], fig["id"]
        per.setdefault(art, []).append(fig)
    for p in sorted(glob.glob(GB + "/body/*.html")):
        slug = os.path.basename(p)[:-5]              # gb-<tail>
        zh = open(p, encoding="utf-8").read()
        en = open("%s/en/%s-en.html" % (GB, slug), encoding="utf-8").read()
