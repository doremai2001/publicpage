# -*- coding: utf-8 -*-
"""Stage the bt fragments for topicbuild: strip the bt- prefix (and, on the
...
the bt English fragments are en/bt-<slug>-en.html.  Both the prefix and the
suffix come off here, so the builder's "<PREFIX>-<slug>.html" composes
cleanly for both languages.

Like lv / brt / pel / ec / gb / em, every bt figure carries
...
PLACEMENT IS THE AUTHORITY.  All twelve bt figures have a single-article
used_by that agrees with placement, so twelve figures make twelve
insertions, one each in A1 / A2 / A3 / A4 / B1 / B3 / B4 / B5 / C2 / D1 /
D4 / E5.

Figures are inserted HERE, into the staged copy the builder reads, so the
source fragments under /home/claude/bt/body and /home/claude/bt/en stay
...
SVG's viewBox height; the bt SVGs carry integer heights, but the pel-style
round-half-up is kept so a fractional one would not break the build.

Set BT_STAGE to match bt.py's _STAGE.
"""
import decimal
import glob
import json
...
BT = "/home/claude/bt"
STAGE = os.environ.get("BT_STAGE", "./staging-bt")
...
def svg_height(name):
    ...
    s = open(BT + "/figs/" + name, encoding="utf-8").read()
...
def stage():
    ...
    manifest = json.load(open(BT + "/figs/manifest.json", encoding="utf-8"))
    per = {}
    for fig in manifest:
    ...
    n_ins = 0
    for p in sorted(glob.glob(BT + "/body/*.html")):
        slug = os.path.basename(p)[:-5]              # bt-<tail>
        zh = open(p, encoding="utf-8").read()
        en = open("%s/en/%s-en.html" % (BT, slug), encoding="utf-8").read()
    ...
    assert n_ins == 2 * len(manifest), n_ins
    return n_ins
