# -*- coding: utf-8 -*-
"""Verification pass over the generated bladder-cancer pages.

The cervix / liver / pel / esoph / gbm / endo / bt standard, printed as one
PASS/FAIL line per check:

  1  body text == source fragment byte-for-byte once the inserted <figure>
     blocks are stripped (zh and en, all 18); <hr> never leaks; h4 and
     citation counts equal source AND equal between zh and en
  2  reference list == source <ol> item by item; meta N == count; [n]
     markers run 1..N in first-use order; zh == en reference counts
  3  figure position per manifest after_h4 (same section index in en); every
     article a figure declares in used_by actually carries it
  4  figure files / alt / caption / height per manifest, escaped exactly once
  5  hreflang zh-Hant / en / x-default + canonical, both directions
  6  language switch chip points at the partner page, both directions
  7  JSON-LD parses on every page; about / name / url / inLanguage / dates
  8  topics.html and topics-en.html differ from prebackup by exactly one
     card (15th, after the bt one) and one appended hasPart entry
  9  sitemap differs from prebackup by exactly the new contiguous block
     (after bt-nf2-en.html, before carc.html), 794 -> 832, priorities
 10  pages produced (38) == sitemap URLs added; every page in the sitemap;
     18 zh == 18 en, groups 4/4/5/2/3
 11  every internal .html link and svg src/srcset in the repo resolves; the
     48 SVGs are byte-identical to /home/claude/bl/figs
 12  the disclosure paragraph sits before the first <h4> of exactly
     bl-two-roads / bl-tmt / bl-tmt-after, byte-identical across the three
     (zh == SPEC.md 二, en == SPEC-EN 1), and appears in no other article
 13  h1 / <title> / og:title / description / ld headline == meta title & dek;
     hub card titles == meta titles; hub tag counts consistent; every
     cross-reference title is verbatim 修正 25 / SPEC-EN section 4 or a live
     page's own title, with no near-miss variant; SPEC-EN section 4's
     fifteen "existing site page" titles still equal those pages' own live
     <title> head
 14  every byte outside the substituted slots equals the cc template; style
     identical across the 38 pages; no double-escaped entities
 15  pel-who.html and pel-who-en.html differ from prebackup by exactly the
     PEL_WHO_EDITS substitutions -- three in Chinese, two in English, five
     in all -- with NO anchor added: in-body site links stay at zero in both
     files, <a> counts unchanged, and nothing else on either page moves
 16  git status shows only the expected new files and the five shared
     modifications -- nothing staged, nothing committed

THE LINK QUESTION goes the OPPOSITE way from the bt round, and deliberately
so.  bt's chore page, gb-what-it-is, already linked lc-brainmet in body, so a
LINKED title was that page's own house form and the bt round added one <a>.
bl's chore page is pel-who, and pel-who is the endo round's page: its body
carries ZERO in-body site links -- every pointer on it is an unlinked
〈title〉 (zh) or "title" (en), including the 〈子宮內膜癌專題〉 /
"Endometrial Cancer Guide" the endometrial round put there.  Re-checked here
rather than inherited: 12 unlinked 〈…〉 pointers and 0 in-body .html links in
the Chinese prebackup, 0 and 0 in the English one.  So this round points in
sense and adds no anchor, and check 15 pins that by value in both
directions: body_links() == [] before AND after, '<a ' and '<a href='
unchanged, href="bl.html" / "bl-en.html" absent, the new pointer present
exactly once in the page's own unlinked form.

ALLOW_ZH_ANGLE is EMPTY for this topic, and that is a finding rather than an
omission: all 70 〈...〉 strings in the 18 Chinese bodies -- 69 in prose plus
one inside the fig-bl-nmibc-risk alt text -- resolve to either a 修正 25
canonical title or a live page's own <h1>.  The bl bodies do quote source
documents, guideline chapter headings and phrases-not-to-use by name, but
they do so inside 「...」 (and, for English-language material, in Latin
script without 〈 〉), so no non-cross-reference 〈...〉 exists to allow-list.

Deviations from verify_bt.py.  All of them are strictly stronger, none
weaker:
  * check 3 drops bt's `len(fig["used_by"]) == 1` and puts a coverage rule in
    its place: every article a figure declares in used_by must actually carry
    that figure, in both languages.  A one-article used_by was a fact about
    the bt manifest, not a house rule -- SPEC 七 / 修正 26 item 9 draw
    fig-bl-followup for B4 AND C5, and the figure really is two columns, one
    per route.  The check that matters is therefore not "how many articles
    may claim a figure" but "does every article that claims one get it".
    This is what caught the second genuine defect of the round: placement
    was a single dict naming bl-nmibc-followup only, so bl-tmt-after was
    built with no figure at all and the stager's own count -- placements,
    not used_by entries -- passed.  placement is a LIST now, one entry per
    article, held to used_by as a set both here and in stage_bl_figs.py, and
    twelve figures make thirteen insertions per language.
  * check 13's English quoted-string scan keeps bt's two improvements -- the
    scan runs over the body with its TAGS REMOVED (bl bodies wrap every
    citation in <a href="https://...">, so a raw scan harvests URL attribute
    values), and it walks consecutive quote PAIRS in both parities rather
    than regex-pairing '"(.{18,110})"' left to right -- and adds two more:
  * the live -en.html titles the scan is checked against are HTML-UNESCAPED
    before comparison.  verify_bt compared the raw <title> head text, which
    happened to work for bt because none of its seventeen pointer pages had
    an apostrophe in the title.  care-mood-en.html does: its head reads
    "The kind of sleeplessness that won&#x27;t lift".  The bodies quote the
    title as prose, so without the unescape a perfectly correct pointer in
    bl-daily-en reads as a 0.92 near-miss and SPEC-EN section 4's own row
    for that page reads as wrong.  Both places unescape now.
  * the cross-reference scan is run over TWO surfaces instead of one.  bt
    scanned the Chinese body as raw markup (so alt attribute text was
    included) but the English body with tags removed (so alt attribute text
    was NOT) -- the two languages were not being held to the same rule, and
    per-article multisets taken that way are not comparable.  Here the
    prose scan strips the <figure> blocks in BOTH languages, so the
    per-article multiset comparison is like for like, and a SECOND scan
    reads the alt attribute and the figcaption of every inserted figure in
    both languages and holds the titles quoted there to the same canonical
    rule.  This is what catches the one genuine defect of the round; see
    check 13's output.
"""
import sys

sys.dont_write_bytecode = True      # keep _pipeline/__pycache__ from coming
                                    # back behind check 16

import decimal
import difflib
import filecmp
import html
import json
import os
import re
import subprocess

sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
import bl
import topicbuild as tb

REPO = "/home/claude/repo"
PRE = "/home/claude/bl/prebackup"
BL = "/home/claude/bl"
NART = 18
NFIG = 12
NSVG = 48
GROUPS = [4, 4, 5, 2, 3]
ORD = [s for sec in bl.SECTIONS for s in sec["slugs"]]
LANGS = [("zh", ""), ("en", "-en")]
ART_TPL = {"zh": "cc-first-month.html", "en": "cc-first-month-en.html"}
HUB_TPL = {"zh": "cc.html", "en": "cc-en.html"}
SM_PREV = "bt-nf2-en.html"
SM_NEXT = "carc.html"
PREV_CARD = {"zh": "bt.html", "en": "bt-en.html"}
NCARD = 15
SM_BEFORE, SM_AFTER = 794, 832
DISC_ARTS = ["two-roads", "tmt", "tmt-after"]
ANCHOR = {"zh": "先說我的位置", "en": "My position first"}
NEW_PIPELINE = ["build_bl.py", "bl.py", "bl_en.py",
                "stage_bl_figs.py", "verify_bl.py"]
SHARED = ["pel-who-en.html", "pel-who.html", "sitemap.xml",
          "topics-en.html", "topics.html"]
NPAGES = 2 + 2 * NART

# Empty on purpose -- see the module docstring.  Every 〈...〉 in the 18
# Chinese bodies (prose and figure alt alike) is a page cross-reference; the
# guidelines, chapter headings and phrases-not-to-use the bodies quote by
# name are quoted in 「...」, which this check does not touch.
ALLOW_ZH_ANGLE = set()

hubs = ["bl.html", "bl-en.html"]
arts = ["bl-%s%s.html" % (s, suf) for _, suf in LANGS for s in ORD]
pages = hubs + arts
svgs = sorted(n for n in os.listdir(os.path.join(BL, "figs"))
              if n.endswith(".svg"))
manifest = json.load(open(os.path.join(BL, "figs", "manifest.json"),
                          encoding="utf-8"))

results = []
_cur = []


def chk(ok, msg):
    if not ok:
        _cur.append(msg)


def report(no, label):
    global _cur
    ok = not _cur
    results.append((no, label, ok, list(_cur)))
    print("%-4s %s -- %s" % ("PASS" if ok else "FAIL", "%2d." % no, label))
    for m in _cur[:12]:
        print("        " + str(m))
    if len(_cur) > 12:
        print("        ... %d more" % (len(_cur) - 12))
    _cur = []


def rd(*p):
    return open(os.path.join(*p), encoding="utf-8").read()


def esca(s):
    return html.escape(s, quote=False).replace('"', "&quot;")


def src_path(slug, lang):
    """The bl fragments keep the bl- prefix in BOTH languages, and the
    English ones carry a -en suffix on top of it (the bt convention, not
    em's, whose English fragments carried neither)."""
    if lang == "zh":
        return os.path.join(BL, "body", "bl-%s.html" % slug)
    return os.path.join(BL, "en", "bl-%s-en.html" % slug)


def page_body(s):
    return re.search(r'<div class="body-html">\n(.*?)\n    </div>', s,
                     re.S).group(1)


def page_refs(s):
    return re.search(r'<div class="refs">.*?<ol>(.*?)</ol>', s, re.S).group(1)


def ld_of(s):
    return json.loads(re.search(
        r'<script type="application/ld\+json">(.*?)</script>', s,
        re.S).group(1))


RE_FIG = re.compile(
    r'<figure class="article-figure">\s*<picture>\s*'
    r'<source media="\(max-width:620px\)" srcset="([^"]+)">\s*'
    r'<img src="([^"]+)" width="1440" height="(\d+)" loading="lazy" '
    r'decoding="async" alt="([^"]*)">\s*</picture>\s*'
    r'<figcaption>(.*?)</figcaption>\s*</figure>', re.S)
RE_FIG_STRIP = re.compile(
    r'\n?<figure class="article-figure">.*?</figure>\n?', re.S)
RE_TAG = re.compile(r"<[^>]+>")


def vb_height(svg_name):
    m = re.search(r'viewBox="0 0 (\d+(?:\.\d+)?) (\d+(?:\.\d+)?)"',
                  rd(BL, "figs", svg_name))
    assert m and m.group(1) == "1440", svg_name
    return str(int(decimal.Decimal(m.group(2)).quantize(
        decimal.Decimal("1"), rounding=decimal.ROUND_HALF_UP)))


for p in pages:
    if not os.path.exists(os.path.join(REPO, p)):
        print("FATAL: missing %s" % p)
        sys.exit(2)

# 1 ------------------------------------------ body == source minus figures --
for slug in ORD:
    counts = {}
    for lang, suf in LANGS:
        body, items, n = tb.split_fragment(rd(src_path(slug, lang)))
        pb = page_body(rd(REPO, "bl-%s%s.html" % (slug, suf)))
        nofig = RE_FIG_STRIP.sub("\n", pb)
        # the figure sits between two sections; stripping it may leave a
        # collapsed blank line, so compare after normalising the separator
        chk(re.sub(r"\n{2,}", "\n\n", nofig).strip()
            == re.sub(r"\n{2,}", "\n\n", body).strip(),
            "bl-%s%s body differs from source outside the figure block"
            % (slug, suf))
        chk("<hr>" not in pb, "bl-%s%s: <hr> leaked into body" % (slug, suf))
        nh4 = len(re.findall(r"<h4>", pb))
        ncit = len(re.findall(r'<sup class="cit">', pb))
        chk(nh4 == len(re.findall(r"<h4>", body)),
            "bl-%s%s: h4 count" % (slug, suf))
        chk(ncit == len(re.findall(r'<sup class="cit">', body)),
            "bl-%s%s: citation count" % (slug, suf))
        counts[lang] = (nh4, ncit)
    chk(counts["zh"][0] == counts["en"][0],
        "bl-%s: zh %d h4 vs en %d" % (slug, counts["zh"][0], counts["en"][0]))
    chk(counts["zh"][1] == counts["en"][1],
        "bl-%s: zh %d citations vs en %d"
        % (slug, counts["zh"][1], counts["en"][1]))
report(1, "body-html == source fragment byte-for-byte after stripping the "
          "inserted <figure> (%d zh + %d en); <hr> never leaks; h4 and "
          "citation counts equal the source and equal each other across "
          "languages" % (NART, NART))

# 2 ------------------------------------------------- references item by item --
for slug in ORD:
    for lang, suf in LANGS:
        body, items, n = tb.split_fragment(rd(src_path(slug, lang)))
        page = rd(REPO, "bl-%s%s.html" % (slug, suf))
        pr = page_refs(page)
        chk(pr == items, "bl-%s%s: reference <ol> content differs from source"
            % (slug, suf))
        src_li = re.findall(r"<li>.*?</li>", items, re.S)
        page_li = re.findall(r"<li>.*?</li>", pr, re.S)
        chk(len(src_li) == len(page_li) == n,
            "bl-%s%s: %d/%d/%d reference items" % (slug, suf, len(src_li),
                                                   len(page_li), n))
        for k, (a, b) in enumerate(zip(src_li, page_li)):
            chk(a == b, "bl-%s%s: reference %d differs" % (slug, suf, k + 1))
        chk("<ol>" not in pr and "<p></p>" not in pr,
            "bl-%s%s: nested <ol> or trailing empty <p>" % (slug, suf))
        mm = re.search(r'<div class="meta">(.*?)</div>', page).group(1)
        chk(int(re.search(r"\d+", mm.split(" · ")[2]).group(0)) == n,
            "bl-%s%s: meta reference count != %d" % (slug, suf, n))
        nums = [int(x) for x in
                re.findall(r'<sup class="cit">\[(\d+)\]</sup>', page_body(page))]
        first = [v for i, v in enumerate(nums) if v not in nums[:i]]
        chk(first == list(range(1, n + 1)),
            "bl-%s%s: [n] markers not 1..%d in first-use order" % (slug, suf, n))
    zn = tb.split_fragment(rd(src_path(slug, "zh")))[2]
    en_ = tb.split_fragment(rd(src_path(slug, "en")))[2]
    chk(zn == en_, "bl-%s: zh %d refs vs en %d" % (slug, zn, en_))
report(2, "reference list == source <ol> item by item on all %d pages; meta "
          "N == count; [n] markers run 1..N in first-use order; zh == en "
          "reference counts" % (2 * NART))

# 3 ------------------------------------------------ figure placement ---------
per = {}
NPLACE = 0
for fig in manifest:
    places = fig["placement"]
    chk(isinstance(places, list) and len(places) >= 1,
        "%s: placement is not a non-empty list" % fig["id"])
    if not isinstance(places, list):
        places = [places]
    pl_arts = [pl["article"] for pl in places]
    chk(sorted(pl_arts) == sorted(fig["used_by"]),
        "%s: placement articles %s != used_by %s"
        % (fig["id"], pl_arts, fig["used_by"]))
    for pl in places:
        per.setdefault(pl["article"], []).append((fig, pl))
    NPLACE += len(places)
chk(len(manifest) == NFIG, "manifest has %d figures, want %d"
    % (len(manifest), NFIG))
nfig = 0
alt_pairs = []
for slug in ORD:
    want = per.get("bl-" + slug, [])
    src_h4 = re.findall(r"<h4>(.*?)</h4>", rd(src_path(slug, "zh")))
    want_pos = []
    for f, pl in want:
        chk(pl["after_h4"] in src_h4,
            "%s: after_h4 not found in bl-%s" % (f["id"], slug))
        if pl["after_h4"] in src_h4:
            want_pos.append(src_h4.index(pl["after_h4"]) + 1)
    for lang, suf in LANGS:
        body = page_body(rd(REPO, "bl-%s%s.html" % (slug, suf)))
        figs = RE_FIG.findall(body)
        chk(len(figs) == len(want), "bl-%s%s: %d figures, want %d"
            % (slug, suf, len(figs), len(want)))
        pos, h4 = [], 0
        for m in re.finditer(r'<h4>|<figure class="article-figure">', body):
            if m.group(0) == "<h4>":
                h4 += 1
            else:
                pos.append(h4)
        chk(pos == want_pos, "bl-%s%s: figure after h4 #%s, want #%s"
            % (slug, suf, pos, want_pos))
        # the figure closes its section: the next thing after it is an <h4>
        for m in re.finditer(r"</figure>\n+", body):
            chk(body[m.end():m.end() + 4] == "<h4>",
                "bl-%s%s: figure is not immediately followed by the next <h4>"
                % (slug, suf))
        for got, (fig, pl) in zip(figs, want):
            alt_pairs.append((lang, slug, fig, got))
        nfig += len(figs)
chk(nfig == NPLACE * 2, "%d figure instances, want %d (%d placements x 2 "
    "languages)" % (nfig, NPLACE * 2, NPLACE))
# EVERY article a figure declares in used_by must actually carry it.  bt
# asserted len(used_by) == 1 instead, which is a bt fact and not a rule: a
# figure drawn to serve two articles is legitimate, but then both of them
# have to receive it.  placement is a LIST here for exactly that reason, and
# is held to used_by as a set above; this block is the independent check,
# read off the built pages rather than off the manifest.
for fig in manifest:
    for art in fig["used_by"]:
        for lang, suf in LANGS:
            want_src = fig["files"]["desktop" if lang == "zh" else "en"]
            p = "%s%s.html" % (art, suf)
            if not os.path.exists(os.path.join(REPO, p)):
                chk(False, "%s: used_by names %s, which is not a page"
                    % (fig["id"], p))
                continue
            chk('src="%s"' % want_src in rd(REPO, p),
                "%s: used_by declares %s but %s carries no such figure "
                "(placement names %s)"
                % (fig["id"], art, p,
                   [q["article"] for q in fig["placement"]]))
report(3, "%d manifest figures -> %d insertions per language, each closing "
          "the h4 section named by its placement.after_h4 (same section index "
          "in en); placement articles == used_by as a set, and every article "
          "a figure declares in used_by actually carries it"
          % (NFIG, NPLACE))

# 4 ---------------------------------------- figure files / alt / caption -----
for lang, slug, fig, got in alt_pairs:
    f = fig["files"]
    mob = f["mobile"] if lang == "zh" else f["en_mobile"]
    dsk = f["desktop"] if lang == "zh" else f["en"]
    raw_alt = fig["zh_alt"] if lang == "zh" else fig["en_alt"]
    raw_cap = fig["zh_caption"] if lang == "zh" else fig["en_caption"]
    tag = "bl-%s (%s)" % (slug, lang)
    chk(got[0] == mob, "%s: srcset %s != %s" % (tag, got[0], mob))
    chk(got[1] == dsk, "%s: src %s != %s" % (tag, got[1], dsk))
    chk(got[2] == vb_height(dsk), "%s: height %s != viewBox %s"
        % (tag, got[2], vb_height(dsk)))
    chk(got[3] == esca(raw_alt), "%s: alt != escaped manifest alt" % tag)
    chk(html.unescape(got[3]) == raw_alt, "%s: alt does not round-trip" % tag)
    chk('"' not in got[3] and "<" not in got[3] and ">" not in got[3],
        "%s: raw quote/angle bracket in alt" % tag)
    chk(got[4] == raw_cap.replace("&", "&amp;").replace("<", "&lt;"),
        "%s: caption != escaped manifest caption" % tag)
    chk("&amp;amp;" not in got[3] + got[4] and "&amp;lt;" not in got[3] + got[4],
        "%s: double-escaped alt/caption" % tag)
report(4, "all %d figure instances: mobile/desktop files per language, "
          "height = desktop viewBox, alt attribute-escaped once, caption "
          "text-escaped once, all equal to the manifest" % (NPLACE * 2))

# 5 ----------------------------------------------------------- hreflang -------
for slug in ORD + [None]:
    z = "bl.html" if slug is None else "bl-%s.html" % slug
    en = "bl-en.html" if slug is None else "bl-%s-en.html" % slug
    for p in (z, en):
        s = rd(REPO, p)
        chk('<link rel="alternate" hreflang="zh-Hant" href="%s%s">'
            % (tb.BASE, z) in s, "%s hreflang zh-Hant" % p)
        chk('<link rel="alternate" hreflang="en" href="%s%s">'
            % (tb.BASE, en) in s, "%s hreflang en" % p)
        chk('<link rel="alternate" hreflang="x-default" href="%s%s">'
            % (tb.BASE, z) in s, "%s hreflang x-default" % p)
        chk('<link rel="canonical" href="%s%s">' % (tb.BASE, p) in s,
            "%s canonical" % p)
        chk(s.count('rel="alternate"') == 3, "%s alternate count" % p)
        chk(s.count('rel="canonical"') == 1, "%s canonical count" % p)
report(5, "hreflang zh-Hant / en / x-default + self-canonical on all %d "
          "pairs, both directions" % (NART + 1))

# 6 -------------------------------------------------------- lang switch ------
for slug in ORD + [None]:
    z = "bl.html" if slug is None else "bl-%s.html" % slug
    en = "bl-en.html" if slug is None else "bl-%s-en.html" % slug
    sz, se = rd(REPO, z), rd(REPO, en)
    chk('<div class="lang"><span class="on">中</span><a href="%s" '
        'hreflang="en">EN</a></div>' % en in sz, "%s lang switch -> %s" % (z, en))
    chk('<div class="lang"><a href="%s" hreflang="zh-Hant">中</a>'
        '<span class="on">EN</span></div>' % z in se,
        "%s lang switch -> %s" % (en, z))
    chk(sz.count('<div class="lang">') == 1
        and se.count('<div class="lang">') == 1,
        "%s/%s lang chip count" % (z, en))
report(6, "language switch chip on every page points at its partner "
          "(zh -> -en, en -> zh), exactly one chip per page")

# 7 --------------------------------------------------------------- JSON-LD ---
for p in pages:
    s = rd(REPO, p)
    try:
        ld = ld_of(s)
    except Exception as e:      # noqa
        chk(False, "%s: JSON-LD does not parse (%s)" % (p, e))
        continue
    en = p.endswith("-en.html")
    chk(ld["@context"] == "https://schema.org", "%s @context" % p)
    chk(ld["url"] == tb.BASE + p, "%s ld url %s" % (p, ld["url"]))
    chk(ld["inLanguage"] == ("en" if en else "zh-Hant"), "%s inLanguage" % p)
    chk(ld["about"]["@type"] == "MedicalCondition", "%s about type" % p)
    chk(ld["about"]["name"] == (bl.CONDITION_EN if en else bl.CONDITION_ZH),
        "%s about.name %s" % (p, ld["about"]["name"]))
    chk(ld["author"]["@type"] == "Physician", "%s author" % p)
    if p in hubs:
        chk(ld["@type"] == "CollectionPage", "%s type" % p)
        chk(ld["name"] == (bl.NAME_EN if en else bl.NAME_ZH), "%s name" % p)
        chk([e["url"] for e in ld["hasPart"]]
            == [tb.BASE + "bl-%s%s.html" % (s_, "-en" if en else "")
                for s_ in ORD], "%s hasPart urls" % p)
        meta = bl.EN if en else bl.ART
        chk([e["name"] for e in ld["hasPart"]]
            == [meta[s_]["title"] for s_ in ORD], "%s hasPart names" % p)
    else:
        slug = p[len("bl-"):-len(".html")]
        if en:
            slug = slug[:-3]
        meta = (bl.EN if en else bl.ART)[slug]
        chk(ld["@type"] == "MedicalWebPage", "%s type" % p)
        chk(ld["headline"] == meta["title"], "%s headline" % p)
        chk(ld["description"] == meta["dek"], "%s description" % p)
        chk(ld["datePublished"] == bl.DATE
            and ld["dateModified"] == bl.DATE, "%s dates" % p)
    chk(s.count('<script type="application/ld+json">') == 1, "%s ld count" % p)
report(7, "JSON-LD parses on all %d pages; @type / url / inLanguage / "
          "about = MedicalCondition %s|%s / headline / dates / hub hasPart "
          "all correct" % (NPAGES, bl.CONDITION_ZH, bl.CONDITION_EN))

# 8 ---------------------------------------------------- topics pages diff ---
for f, card, hp, href, prev_href in (
        ("topics.html", bl.TOPIC_CARD_ZH, bl.HASPART_ZH, "bl.html",
         PREV_CARD["zh"]),
        ("topics-en.html", bl.TOPIC_CARD_EN, bl.HASPART_EN,
         "bl-en.html", PREV_CARD["en"])):
    pre, now = rd(PRE, f), rd(REPO, f)
    d = list(difflib.unified_diff(pre.splitlines(True), now.splitlines(True),
                                  n=0))
    added = [l[1:] for l in d if l.startswith("+") and not l.startswith("+++")]
    removed = [l[1:] for l in d if l.startswith("-") and not l.startswith("---")]
    entry = "," + json.dumps(hp, ensure_ascii=False, separators=(",", ":"))
    chk(len(removed) == 1 and '"hasPart":[' in removed[0],
        "%s: %d removed lines (want only the JSON-LD line)" % (f, len(removed)))
    ld_new = [l for l in added if '"hasPart":[' in l]
    chk(len(ld_new) == 1 and removed and
        ld_new[0] == removed[0].replace("}]}</script>", "}%s]}</script>" % entry),
        "%s: JSON-LD line not just extended by the bl hasPart entry" % f)
    rest = [l for l in added if '"hasPart":[' not in l]
    chk("".join(rest) == card, "%s: added text is not exactly the bl card" % f)
    grid = re.search(r'<div class="topicgrid">(.*?)\n  </div>', now, re.S).group(1)
    cards = re.findall(r'<a class="topiccard" href="([^"]+)"', grid)
    chk(len(cards) == NCARD and cards[-1] == href and cards[-2] == prev_href,
        "%s: card order %s" % (f, cards))
    chk(now.count('class="topiccard"') == pre.count('class="topiccard"') + 1,
        "%s: card count" % f)
    ld, pre_ld = ld_of(now), ld_of(pre)
    chk(ld["hasPart"][:-1] == pre_ld["hasPart"] and ld["hasPart"][-1] == hp,
        "%s: hasPart not old + bl" % f)
    chk('<div class="k">%s</div>' % bl.KICKER in card, "%s: card kicker" % f)
    chk('<span class="n">%d %s</span>' % (NART, "篇" if f == "topics.html"
                                          else "articles") in card,
        "%s: card count label" % f)
report(8, "topics.html and topics-en.html differ from prebackup by exactly "
          "the bl card (after the bt card, %dth) and one appended hasPart "
          "entry; card says %s / %d" % (NCARD, bl.KICKER, NART))

# 9 --------------------------------------------------------- sitemap diff ---
sm, pre_sm = rd(REPO, "sitemap.xml"), rd(PRE, "sitemap.xml")
RE_LOC = re.compile(re.escape(tb.BASE) + r"([^<]*)</loc>")
RE_BL = re.compile(r"<loc>%s(bl\.html|bl-)" % re.escape(tb.BASE))
sml = sm.splitlines()
bl_lines = [l for l in sml if RE_BL.search(l)]
kept = [l for l in sml if not RE_BL.search(l)]
chk(kept == pre_sm.splitlines(), "non-bl sitemap lines changed or reordered")
chk(not any(RE_BL.search(l) for l in pre_sm.splitlines()),
    "prebackup already had bl lines")
chk(pre_sm.count("<url>") == SM_BEFORE, "prebackup <url> = %d, want %d"
    % (pre_sm.count("<url>"), SM_BEFORE))
chk(sm.count("<url>") == SM_AFTER, "sitemap <url> = %d, want %d"
    % (sm.count("<url>"), SM_AFTER))
chk(len(bl_lines) == NPAGES, "%d bl sitemap lines, want %d"
    % (len(bl_lines), NPAGES))
want_order = (["bl.html", "bl-en.html"] + ["bl-%s.html" % s for s in ORD]
              + ["bl-%s-en.html" % s for s in ORD])
chk([RE_LOC.search(l).group(1) for l in bl_lines] == want_order,
    "bl block order is not hub zh, hub en, %d zh, %d en" % (NART, NART))
prio = {"bl.html": "0.85", "bl-en.html": "0.75"}
prio.update(("bl-%s.html" % s, "0.75") for s in ORD)
prio.update(("bl-%s-en.html" % s, "0.65") for s in ORD)
for l in bl_lines:
    n = RE_LOC.search(l).group(1)
    chk("<priority>%s</priority>" % prio[n] in l, "%s priority" % n)
    chk("<lastmod>%s</lastmod>" % bl.DATE in l, "%s lastmod" % n)
    chk("<changefreq>monthly</changefreq>" in l, "%s changefreq" % n)
i_prev = [i for i, l in enumerate(sml)
          if "<loc>%s%s</loc>" % (tb.BASE, SM_PREV) in l]
chk(len(i_prev) == 1, "anchor %s not found once" % SM_PREV)
if i_prev:
    i = i_prev[0]
    chk(all(RE_BL.search(l) for l in sml[i + 1:i + 1 + NPAGES]),
        "bl block not contiguous right after %s" % SM_PREV)
    chk("<loc>%s%s</loc>" % (tb.BASE, SM_NEXT) in sml[i + 1 + NPAGES],
        "bl block not immediately before %s" % SM_NEXT)
chk("bl-bl-" not in sm and not any("bl-bl-" in n for n in os.listdir(REPO)),
    "double-prefixed name somewhere")
report(9, "sitemap differs from prebackup by exactly one contiguous %d-line "
          "block after %s / before %s; <url> %d -> %d; order hub zh, hub en, "
          "%d zh, %d en; priorities 0.85/0.75/0.75/0.65; lastmod %s"
          % (NPAGES, SM_PREV, SM_NEXT, SM_BEFORE, SM_AFTER, NART, NART,
             bl.DATE))

# 10 ---------------------------------------------- pages == sitemap adds ----
sm_names = set(m.group(1) for m in RE_LOC.finditer(sm))
pre_names = set(m.group(1) for m in RE_LOC.finditer(pre_sm))
added_names = sm_names - pre_names
chk(len(pages) == NPAGES, "%d pages, want %d" % (len(pages), NPAGES))
chk(added_names == set(pages), "sitemap additions != produced pages: +%s -%s"
    % (sorted(added_names - set(pages)), sorted(set(pages) - added_names)))
chk(len(ORD) == NART and [len(s["slugs"]) for s in bl.SECTIONS] == GROUPS,
    "reading order is not %d in %s" % (NART, GROUPS))
chk([len(s["slugs"]) for s in bl.SECTIONS_EN] == GROUPS,
    "English group sizes differ from the Chinese")
chk([s["slugs"] for s in bl.SECTIONS] == [s["slugs"] for s in bl.SECTIONS_EN],
    "zh and en reading orders differ")
chk(len([p for p in arts if p.endswith("-en.html")])
    == len([p for p in arts if not p.endswith("-en.html")]) == NART,
    "zh/en article counts unequal")
repo_html = set(n for n in os.listdir(REPO) if n.endswith(".html"))
chk(all(n in repo_html for n in sm_names if n.endswith(".html")),
    "sitemap URL without a file: %s"
    % sorted(n for n in sm_names if n.endswith(".html") and n not in repo_html))
report(10, "%d pages produced (2 hubs + %d zh + %d en, groups %s, same "
           "order in both languages) == the %d sitemap URLs added; every "
           "sitemap URL has a file"
           % (NPAGES, NART, NART, "/".join(str(g) for g in GROUPS), NPAGES))

# 11 ------------------------------------------------------ internal links ---
files = set(os.listdir(REPO))
bad, nlinks, nimg = [], 0, 0
for name in sorted(files):
    if name.endswith(".html"):
        s = rd(REPO, name)
        for t in re.findall(r'href="([^":]+\.html)(?:[#?][^"]*)?"', s):
            nlinks += 1
            if t not in files:
                bad.append((name, t))
        for t in re.findall(r'(?:src|srcset)="([^"]+\.svg)"', s):
            nimg += 1
            if t not in files:
                bad.append((name, t))
chk(not bad, "broken links: %s" % bad[:10])
# bl pages specifically: every svg they reference is a bl svg
for p in pages:
    s = rd(REPO, p)
    for t in re.findall(r'(?:src|srcset)="([^"]+\.svg)"', s):
        chk(t in svgs, "%s references non-bl svg %s" % (p, t))
for n in svgs:
    chk(os.path.exists(os.path.join(REPO, n)), "svg missing in repo: %s" % n)
    chk(filecmp.cmp(os.path.join(BL, "figs", n), os.path.join(REPO, n),
                    shallow=False), "svg differs in repo: %s" % n)
chk(len(svgs) == NSVG, "%d svgs, want %d" % (len(svgs), NSVG))
report(11, "%d internal .html links and %d svg src/srcset across the repo all "
           "resolve (0 broken); the %d bl SVGs are byte-identical to "
           "bl/figs" % (nlinks, nimg, NSVG))

# 12 --------------------------------------------------------- disclosure ----
for lang, suf in LANGS:
    discs = {}
    for slug in ORD:
        body = page_body(rd(REPO, "bl-%s%s.html" % (slug, suf)))
        head = body[:body.index("<h4>")]
        hits = [p_ for p_ in re.findall(r"<p>.*?</p>", head, re.S)
                if ANCHOR[lang] in p_]
        whole = ANCHOR[lang] in body
        if slug in DISC_ARTS:
            chk(len(hits) == 1, "bl-%s%s: disclosure missing before first <h4>"
                % (slug, suf))
            chk(body.count(ANCHOR[lang]) == 1,
                "bl-%s%s: disclosure anchor appears more than once"
                % (slug, suf))
            if hits:
                discs[slug] = hits[0]
        else:
            chk(not whole, "bl-%s%s: disclosure text present but not allowed"
                % (slug, suf))
    chk(len(set(discs.values())) == 1 and len(discs) == len(DISC_ARTS),
        "%s disclosure not byte-identical across C1/C4/C5 (%d variants)"
        % (lang, len(set(discs.values()))))
    if lang == "en" and discs:
        spec = rd(BL, "SPEC-EN.md")
        want = spec[spec.index("> <p>My position first"):].split("\n", 1)[0][2:]
        chk(list(discs.values())[0] == want,
            "en disclosure != SPEC-EN.md section 1")
    if lang == "zh" and discs:
        spec = rd(BL, "SPEC.md")
        blk = spec[spec.index("> 先說我的位置"):]
        blk = blk[:blk.index("\n\n")]
        want = "".join(l.lstrip("> ").strip() for l in blk.splitlines())
        chk(list(discs.values())[0] == "<p>%s</p>" % want,
            "zh disclosure != SPEC.md section 二")
report(12, "disclosure paragraph before the first <h4> of exactly "
           "bl-two-roads / bl-tmt / bl-tmt-after, byte-identical across the "
           "three (zh == SPEC 二, en == SPEC-EN 1), absent from the other "
           "%d -- both languages" % (NART - len(DISC_ARTS)))

# 13 ----------------------------------------- titles, meta & cross-references --
for p in arts:
    s = rd(REPO, p)
    en = p.endswith("-en.html")
    slug = p[len("bl-"):-len(".html")]
    if en:
        slug = slug[:-3]
    m = (bl.EN if en else bl.ART)[slug]
    si = [i for i, sec in enumerate(bl.SECTIONS) if slug in sec["slugs"]][0]
    chk("<h1>%s</h1>" % tb.esc(m["title"]) in s, "%s h1" % p)
    title = re.search(r"<title>(.*?)</title>", s, re.S).group(1)
    chk(tb.esca(m["title"]) in title, "%s <title>" % p)
    chk(tb.esca(bl.NAME_EN if en else bl.NAME_ZH) in title,
        "%s <title> lacks topic name" % p)
    chk('<meta property="og:title" content="%s">' % title in s, "%s og:title" % p)
    chk('<meta name="description" content="%s">' % tb.esca(m["dek"]) in s,
        "%s description" % p)
    chk('<meta property="og:description" content="%s">' % tb.esca(m["dek"]) in s,
        "%s og:description" % p)
    chk('<p class="dek">%s</p>' % tb.esc(m["dek"]) in s, "%s dek" % p)
    chk('<div class="leadbox"><p>%s</p></div>' % tb.esc(m["lead"]) in s,
        "%s lead" % p)
    chk(tb.esc(m["note"]) in s, "%s mdnote" % p)
    kicker = bl.SECTIONS_EN[si]["en"].upper()
    chk('<div class="kicker">%s</div>' % tb.esc(kicker) in s, "%s kicker" % p)
    sec_name = (bl.SECTIONS_EN[si]["en"] if en else bl.SECTIONS[si]["zh"])
    mm = re.search(r'<div class="meta">(.*?)</div>', s).group(1).split(" · ")
    chk(mm[0] == (bl.NAME_EN if en else bl.NAME_ZH) and mm[1] == sec_name,
        "%s meta line %s" % (p, mm))
    chk(('<a href="topics-en.html" class="on">' if en
         else '<a href="topics.html" class="on">') in s, "%s nav active" % p)
    chk('class="backlink" href="%s"' % ("bl-en.html" if en else "bl.html") in s,
        "%s backlink" % p)
    for k in bl.ART[slug]["tags"]:
        chk('href="%s?tag=%s"' % ("bl-en.html" if en else "bl.html", k) in s,
            "%s tag chip %s" % (p, k))
    # pnav chain
    i = ORD.index(slug)
    pn = re.search(r'<div class="pnav">(.*?)</div>', s, re.S).group(1)
    if i == 0:
        chk(pn.startswith("<span></span>"), "%s first has prev" % p)
    else:
        chk('class="pv" href="bl-%s%s.html"' % (ORD[i - 1], "-en" if en else "")
            in pn, "%s prev" % p)
    if i == NART - 1:
        chk(pn.endswith("<span></span>"), "%s last has next" % p)
    else:
        chk('class="nx" href="bl-%s%s.html"' % (ORD[i + 1], "-en" if en else "")
            in pn, "%s next" % p)
for lang, page in (("zh", "bl.html"), ("en", "bl-en.html")):
    s = rd(REPO, page)
    hub = bl.HUB if lang == "zh" else bl.HUB_EN
    chk("<title>%s</title>" % tb.esca(hub["title"]) in s, "%s title" % page)
    chk('<meta name="description" content="%s">' % tb.esca(hub["desc"]) in s,
        "%s description" % page)
    chk('<div class="kicker">%s</div>' % bl.KICKER in s, "%s kicker" % page)
    chk(tb.esc(hub["intro"]) in s or hub["intro"] in s, "%s intro" % page)
    chk(hub["closing"] in s, "%s closing" % page)
    meta_l = bl.ART if lang == "zh" else bl.EN
    cards = re.findall(r'<a class="postcard" data-tags="([^"]*)" href="([^"]*)"', s)
    chk([c[1] for c in cards] == ["bl-%s%s.html" % (s_, "-en" if lang == "en"
                                                    else "") for s_ in ORD],
        "%s card order" % page)
    for slug in ORD:
        chk('<div class="t">%s</div>' % tb.esc(meta_l[slug]["title"]) in s,
            "%s card title %s" % (page, slug))
    grp = re.findall(r'<div class="postgroup hnstep">.*?\n  </div>', s, re.S)
    chk([g.count('class="postcard"') for g in grp] == GROUPS,
        "%s group sizes" % page)
    for si, sec in enumerate(bl.SECTIONS):
        nm = sec["zh"] if lang == "zh" else bl.SECTIONS_EN[si]["en"]
        chk("<h3><b>%d</b>%s</h3>" % (si + 1, tb.esc(nm)) in s,
            "%s group title %d" % (page, si + 1))
    bar = tb.RE_HUB_TAGC.search(s).group(2)
    cnts = dict((k, int(n)) for k, n in
                re.findall(r'data-tag="([^"]*)">[^<]*<i>(\d+)</i>', bar))
    want = {"": NART}
    for slug in ORD:
        for k in bl.ART[slug]["tags"]:
            want[k] = want.get(k, 0) + 1
    chk(cnts == want, "%s tagbar counts %s" % (page, cnts))
    li = 0 if lang == "zh" else 1
    for k, lab in bl.LABEL_ADD.items():
        chk('data-tag="%s">#%s <i>' % (k, tb.esc(lab[li])) in bar,
            "%s new label %s" % (page, k))

# cross-reference titles: 修正 25 (zh) / SPEC-EN section 4 (en), verbatim
zh_titles = set(bl.ART[s]["title"] for s in ORD)
en_titles = set(bl.EN[s]["title"] for s in ORD)
spec = rd(BL, "SPEC.md")
tbl = spec[spec.index("### 修正 25"):spec.index("### 修正 26")]
fix25 = dict(re.findall(r"\| \`(bl-[a-z0-9-]+)\` \| (.+?) \|", tbl))
chk(len(fix25) == NART, "修正 25 table has %d rows" % len(fix25))
for slug in ORD:
    chk(bl.ART[slug]["title"] == fix25.get("bl-" + slug),
        "bl-%s title != 修正 25 table" % slug)
sp_en = rd(BL, "SPEC-EN.md")
sec4 = sp_en[sp_en.index("## 4. Canonical English titles"):
             sp_en.index("## 5. Structure")]
spec_en_titles = dict(re.findall(r"- `(bl-[a-z0-9-]+)` → \"(.+?)\"", sec4))
chk(len(spec_en_titles) == NART,
    "SPEC-EN section 4 has %d canonical rows" % len(spec_en_titles))
for slug in ORD:
    chk(bl.EN[slug]["title"] == spec_en_titles.get("bl-" + slug),
        "bl-%s en title != SPEC-EN section 4" % slug)

# SPEC-EN section 4 also fixes the English titles for the fifteen ALREADY
# LIVE pages this topic points at.  Re-derive them from the live pages so a
# stale spec cannot bless a stale body.  The live head text is UNESCAPED
# before comparison: care-mood-en.html's <title> reads "won&#x27;t", and the
# spec (like the bodies) quotes the title as prose.
NPTR = 15
ptr = sec4[sec4.index("**Pointers to existing site pages"):]
ptr_rows = re.findall(r"→ \"(.+?)\" \(`([a-z0-9\-]+\.html)`\)", ptr)
chk(len(ptr_rows) == NPTR, "SPEC-EN section 4 lists %d live-page pointers, "
                           "want %d" % (len(ptr_rows), NPTR))
for want_t, fn in ptr_rows:
    if not os.path.exists(os.path.join(REPO, fn)):
        chk(False, "SPEC-EN pointer names a page that does not exist: %s" % fn)
        continue
    head = html.unescape(
        re.split(r"[｜|]", re.search(r"<title>(.*?)</title>",
                                     rd(REPO, fn), re.S).group(1))[0].strip())
    chk(head == want_t, "SPEC-EN pointer title for %s is %r, the live page's "
                        "own title is %r" % (fn, want_t, head))

live_h1, live_head = {}, {}
for n in os.listdir(REPO):
    if not n.endswith(".html") or n == "bl.html" or n.startswith("bl-"):
        continue
    s = rd(REPO, n)
    m = re.search(r"<h1>(.*?)</h1>", s, re.S)
    if m:
        live_h1.setdefault(m.group(1), n)
    if n.endswith("-en.html"):
        m = re.search(r"<title>(.*?)</title>", s, re.S)
        if m:
            live_head.setdefault(
                html.unescape(re.split(r"[｜|]", m.group(1))[0].strip()),
                n[:-len("-en.html")] + ".html")
zh_key = dict((bl.ART[s]["title"], s) for s in ORD)
en_key = dict((bl.EN[s]["title"], s) for s in ORD)


def en_quoted(text):
    """Every span between two consecutive quote marks, both parities.

    verify_endo.py used a plain '"(.{18,110})"' scan, which pairs quotes
    left to right and therefore loses a title whenever an odd number of
    scare quotes precedes it in the same paragraph.  Walking consecutive
    quote PAIRS covers both parities, so no cross-reference can hide behind
    a scare quote -- and the bl bodies are full of scare-quoted phrases the
    author is telling the reader NOT to use."""
    q = [m.start() for m in re.finditer('"', text)]
    return [text[a + 1:b] for a, b in zip(q, q[1:])]


known_en = en_titles | set(live_head)


def en_scan(text, where):
    """Count the canonical titles quoted in a piece of English TEXT (tags
    already removed, entities already resolved), failing on a near miss."""
    seen = {}
    for q in en_quoted(text):
        if not 18 <= len(q) <= 110:
            continue
        if q in known_en:
            k = en_key.get(q) or live_head[q]
            seen[k] = seen.get(k, 0) + 1
            continue
        ratio, near = max((difflib.SequenceMatcher(None, q, k).ratio(), k)
                          for k in known_en)
        chk(ratio < 0.86,
            "%s: quoted \"%s\" is a near-miss of the canonical \"%s\" (%.2f)"
            % (where, q, near, ratio))
    return seen


def zh_scan(body, where):
    seen = {}
    for t in re.findall(r"〈([^〉]*)〉", body):
        if t in zh_titles or t in live_h1:
            k = zh_key.get(t) or live_h1[t]
            seen[k] = seen.get(k, 0) + 1
            continue
        chk(t in ALLOW_ZH_ANGLE,
            "%s: cross-reference 〈%s〉 matches no canonical title" % (where, t))
    return seen


# (a) prose.  The <figure> blocks are stripped in BOTH languages so that the
#     per-article multisets are comparable: bt scanned the Chinese body as
#     raw markup (alt text included) and the English body with tags removed
#     (alt text excluded), which is not the same surface.
n_zh_xref = n_en_xref = 0
zh_refs, en_refs = {}, {}
for slug in ORD:
    body = RE_FIG_STRIP.sub("\n", page_body(rd(REPO, "bl-%s.html" % slug)))
    zh_refs[slug] = zh_scan(body, "bl-%s" % slug)
    n_zh_xref += sum(zh_refs[slug].values())
for slug in ORD:
    body = RE_FIG_STRIP.sub("\n", page_body(rd(REPO, "bl-%s-en.html" % slug)))
    # bl bodies wrap every citation in <a href="https://...">, so the quoted
    # strings are read out of the TEXT, not the markup -- otherwise every URL
    # attribute value would be scanned as if it were a quoted title.
    en_refs[slug] = en_scan(html.unescape(RE_TAG.sub("", body)),
                            "bl-%s-en" % slug)
    n_en_xref += sum(en_refs[slug].values())
# the two languages must point at the same pages, the same number of times
for slug in ORD:
    chk(zh_refs[slug] == en_refs[slug],
        "bl-%s: cross-references differ between languages, zh %s vs en %s"
        % (slug, sorted(zh_refs[slug].items()), sorted(en_refs[slug].items())))
chk(n_zh_xref >= NART and n_en_xref >= NART,
    "suspiciously few cross-references found (zh %d, en %d)"
    % (n_zh_xref, n_en_xref))

# (b) the inserted figures.  A title quoted inside a figure's alt text or
#     caption is a cross-reference like any other and is held to the same
#     canonical rule, in both languages.  (bt reached the Chinese alt text by
#     accident, through its raw-markup zh scan, and never reached the English
#     one at all -- an <img alt="..."> is inside a tag, so tag-stripping
#     removes it.)
n_zh_fig = n_en_fig = 0
for slug in ORD:
    for lang, suf in LANGS:
        body = page_body(rd(REPO, "bl-%s%s.html" % (slug, suf)))
        for m in re.finditer(
                r'<figure class="article-figure">.*?</figure>', body, re.S):
            blk = m.group(0)
            alt = html.unescape(re.search(r'alt="([^"]*)"', blk).group(1))
            cap = html.unescape(re.search(r"<figcaption>(.*?)</figcaption>",
                                          blk, re.S).group(1))
            where = "bl-%s%s figure" % (slug, suf)
            if lang == "zh":
                n_zh_fig += sum(zh_scan(alt + "\n" + cap, where).values())
            else:
                n_en_fig += sum(en_scan(alt + "\n" + cap, where).values())
report(13, "h1 / <title> / og:title / description / dek / lead / note / "
           "kicker / meta line / tag chips / pnav chain == meta & module on "
           "all %d articles; hub title/desc/intro/closing, 5 groups %s, "
           "card titles = meta titles, tag counts = card counts, new label "
           "urostomy rendered; %d zh and %d en cross-reference titles in "
           "prose (plus %d zh / %d en inside figure alt and caption text) "
           "are verbatim 修正 25 / SPEC-EN 4 or a live page's own title, no "
           "near-miss variants, and every article points at the same pages "
           "the same number of times in both languages; SPEC-EN 4's %d "
           "live-page titles still match those pages"
           % (2 * NART, "/".join(str(g) for g in GROUPS), n_zh_xref,
              n_en_xref, n_zh_fig, n_en_fig, NPTR))

# 14 -------------------------------------- skeleton / style / escaping ------
def skeleton(s):
    for rx in (tb.RE_TITLE, tb.RE_DESC, tb.RE_OGT, tb.RE_OGD, tb.RE_OGU,
               tb.RE_CANON, tb.RE_LD, tb.RE_STYLE, tb.RE_ALTS, tb.RE_LANG):
        s = rx.sub("@", s)
    s = re.sub(r'(<div class="article">).*?(\n  </div>\n</section>)',
               r"\1@\2", s, flags=re.S)
    s = re.sub(r'<section class="band narrow">.*?\n</section>', "@", s,
               flags=re.S)
    return s


for lang, suf in LANGS:
    t = skeleton(rd(REPO, ART_TPL[lang]))
    for slug in ORD:
        chk(skeleton(rd(REPO, "bl-%s%s.html" % (slug, suf))) == t,
            "bl-%s%s: skeleton differs from %s" % (slug, suf, ART_TPL[lang]))
for lang, page in (("zh", "bl.html"), ("en", "bl-en.html")):
    chk(skeleton(rd(REPO, page)) == skeleton(rd(REPO, HUB_TPL[lang])),
        "%s: skeleton differs from %s" % (page, HUB_TPL[lang]))
tpl_style = tb.RE_STYLE.search(rd(REPO, ART_TPL["zh"])).group(0)
chk(".article-figure{" in tpl_style,
    "cc template no longer carries the figure css -- inject_css path changed")
styles = set()
for p in pages:
    s = rd(REPO, p)
    st = tb.RE_STYLE.search(s).group(0)
    styles.add(st)
    chk(st == tpl_style, "%s: style != template" % p)
    chk(s.count(".article-figure{") == 1, "%s: figure css count" % p)
    for tag in ("<style>", "</style>", "<body>", "</body>", "</html>",
                "<head>", "</head>"):
        chk(s.count(tag) == 1, "%s: %s x%d" % (p, tag, s.count(tag)))
    chk("&amp;amp;" not in s and "&amp;quot;" not in s and "&#x27;" not in s
        and "&amp;gt;" not in s and "&amp;lt;" not in s,
        "%s: double-escaped entity" % p)
chk(len(styles) == 1, "style not byte-stable across pages")
report(14, "every byte outside the substituted slots equals the cc template; "
           "<style> byte-identical on all %d (figure css present once); "
           "head/body tags once; no double-escaped entities" % NPAGES)

# 15 ------------------------------------------------------- pel-who diff ----
def body_links(s):
    b = re.search(r'<div class="body-html">\n(.*?)\n    </div>',
                  s, re.S).group(1)
    return re.findall(r'href="([^":]+\.html)(?:[#?][^"]*)?"', b)


chk(sorted(bl.PEL_WHO_EDITS) == ["pel-who-en.html", "pel-who.html"],
    "PEL_WHO_EDITS covers %s" % sorted(bl.PEL_WHO_EDITS))
WANT_EDITS = {"pel-who.html": 3, "pel-who-en.html": 2}
n_subs = 0
for f in ("pel-who.html", "pel-who-en.html"):
    pre, now = rd(PRE, f), rd(REPO, f)
    pl, nl = pre.splitlines(True), now.splitlines(True)
    chk(len(pl) == len(nl), "%s: line count changed %d -> %d"
        % (f, len(pl), len(nl)))
    diff_idx = [i for i, (a, b) in enumerate(zip(pl, nl)) if a != b]
    edits = bl.PEL_WHO_EDITS[f]
    chk(len(edits) == WANT_EDITS[f], "%s: %d edits declared, want %d"
        % (f, len(edits), WANT_EDITS[f]))
    # one substitution per sentence, each in its own paragraph, so the number
    # of changed lines equals the number of substitutions
    chk(len(diff_idx) == WANT_EDITS[f], "%s: %d changed lines, want %d "
        "(lines %s)" % (f, len(diff_idx), WANT_EDITS[f],
                        [i + 1 for i in diff_idx]))
    want = pre
    for old, new in edits:
        chk(want.count(old) == 1, "%s: anchor not present exactly once in the "
            "prebackup: %r" % (f, old[:40]))
        want = want.replace(old, new)
        n_subs += 1
    chk(now == want, "%s: content is not the prebackup with exactly the "
        "declared pel-who substitutions applied" % f)
    for old, new in edits:
        chk(old not in now, "%s: old wording still present: %r" % (f, old[:40]))
        chk(now.count(new) == 1, "%s: new wording not present exactly once: %r"
            % (f, new[:40]))
    # THE LINK QUESTION, checked rather than inherited.  pel-who's body
    # carries ZERO in-body site links: every pointer on the page is an
    # unlinked 〈title〉 (zh) or "title" (en), including the endometrial
    # round's own.  So this round adds NO anchor, and the invariant is a
    # value on both sides of the diff rather than "one more than before".
    chk(body_links(pre) == [],
        "%s: prebackup already had in-body site links %s -- the premise of "
        "this check no longer holds, re-decide the link question"
        % (f, body_links(pre)))
    chk(body_links(now) == [],
        "%s: in-body site links %s, want none" % (f, body_links(now)))
    chk(now.count("<a ") == pre.count("<a ")
        and now.count("<a href=") == pre.count("<a href="),
        "%s: anchor count %d -> %d, want unchanged"
        % (f, pre.count("<a href="), now.count("<a href=")))
    chk('href="bl.html"' not in now and 'href="bl-en.html"' not in now
        and 'href="bl-' not in now,
        "%s: a bl link was added -- this round points in sense only" % f)
chk(n_subs == 5, "%d substitutions declared in all, want 5" % n_subs)
# The new pointer is named exactly once per file, in that page's own
# unlinked house form, and the endometrial round's pointer is untouched.
zpw, epw = rd(REPO, "pel-who.html"), rd(REPO, "pel-who-en.html")
pre_zpw, pre_epw = rd(PRE, "pel-who.html"), rd(PRE, "pel-who-en.html")
chk(zpw.count(bl.NAME_ZH) == 1,
    "pel-who.html names %s %d times, want 1" % (bl.NAME_ZH,
                                                zpw.count(bl.NAME_ZH)))
chk(zpw.count("〈%s〉" % bl.NAME_ZH) == 1,
    "pel-who.html: the pointer is not the unlinked 〈title〉 form this page "
    "uses for every other pointer")
chk(epw.count(bl.NAME_EN) == 1,
    'pel-who-en.html names "%s" %d times, want 1'
    % (bl.NAME_EN, epw.count(bl.NAME_EN)))
chk(epw.count('"%s"' % bl.NAME_EN) == 1,
    'pel-who-en.html: the pointer is not the unlinked "title" form this page '
    "uses for every other pointer")
chk(zpw.count("〈子宮內膜癌專題〉") == pre_zpw.count("〈子宮內膜癌專題〉") == 1,
    "pel-who.html: the endometrial round's own pointer moved")
chk(epw.count('"Endometrial Cancer Guide"')
    == pre_epw.count('"Endometrial Cancer Guide"') == 1,
    "pel-who-en.html: the endometrial round's own pointer moved")
# the page no longer promises a topic it now has
for bad in ("膀胱癌另有專題規劃中", "還在規劃中"):
    chk(bad not in zpw, "pel-who.html still promises the topic: %r" % bad)
for bad in ("bladder cancer topic is being planned",
            "bladder cancer topic is still being planned"):
    chk(bad not in epw, "pel-who-en.html still promises the topic: %r" % bad)
# ... and the pelvic-radiotherapy facts around the rewritten sentences are
# byte-identical: same paragraph, same citations, same numbers
chk(zpw.count('<sup class="cit">') == pre_zpw.count('<sup class="cit">'),
    "pel-who.html: citation count changed")
chk(epw.count('<sup class="cit">') == pre_epw.count('<sup class="cit">'),
    "pel-who-en.html: citation count changed")
report(15, "pel-who.html and pel-who-en.html differ from prebackup by "
           "exactly the PEL_WHO_EDITS substitutions -- three in Chinese, two "
           "in English, five in all, one changed line per substitution -- "
           "and NO anchor is added: in-body site links stay at zero in both "
           "files, the <a> "
           "counts are unchanged, the new pointer appears once in the page's "
           "own unlinked form, the endometrial round's pointer is untouched, "
           "and nothing else on either page moved")

# 16 --------------------------------------------------------- git status ----
st = subprocess.run(["git", "-C", REPO, "status", "--short", "--porcelain"],
                    capture_output=True, text=True).stdout.splitlines()
mod = sorted(l[3:] for l in st if l.startswith(" M") or l.startswith("M "))
new = sorted(l[3:] for l in st if l.startswith("??"))
other = [l for l in st if not (l.startswith(" M") or l.startswith("M ")
                               or l.startswith("??"))]
staged = [l for l in st if l[0] not in " ?"]
want_new = sorted(pages + svgs + ["_pipeline/" + n for n in NEW_PIPELINE])
chk(mod == SHARED, "modified files: %s" % mod)
chk(new == want_new, "untracked: extra %s / missing %s"
    % (sorted(set(new) - set(want_new)), sorted(set(want_new) - set(new))))
chk(not other, "unexpected git status entries: %s" % other)
chk(not staged, "something is staged for commit: %s" % staged)
head = subprocess.run(["git", "-C", REPO, "log", "-1", "--format=%s"],
                      capture_output=True, text=True).stdout.strip()
chk(not head.lower().startswith("bl") and "bl:" not in head.lower()
    and "膀胱癌" not in head,
    "HEAD looks like a bl commit -- this round is local only: %r" % head)
report(16, "git status: exactly %s modified; untracked = %d pages + %d svgs + "
           "5 _pipeline modules; nothing staged, nothing committed"
           % (", ".join(SHARED), NPAGES, NSVG))

print()
nfail = sum(1 for r in results if not r[2])
if nfail:
    print("FAILED: %d of %d checks" % (nfail, len(results)))
    sys.exit(1)
print("ALL %d CHECKS PASSED" % len(results))
