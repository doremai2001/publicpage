# -*- coding: utf-8 -*-
"""Build the pelvic-radiotherapy topic into the working clone, patch the topic
index page (card + JSON-LD hasPart) and the sitemap, copy the figure SVGs in,
mirror everything new/changed into the upload directory, and report the counts
plus an internal-link scan.

Same driver as build_brt.py.  Differences for pel:

* pel.ZH_ONLY drives the round.  True (round 1): 13 Chinese articles +
  pel.html, no hreflang / English canonical partner / language-switch chip,
  topics.html only, 14 sitemap URLs.  False (round 2, ACTIVE): LANGS becomes
  ("zh", "en"), all 28 pages are rewritten with hreflang and the language
  switch back on the Chinese ones, topics-en.html gets its card, and the
  sitemap picks up the 14 English URLs.  Both patchers are idempotent, so
  round 2 leaves topics.html and the already-present sitemap lines alone.
* The baseline for "what changed" is the git HEAD blob of each file.  Round 1
  is published as commits 55610da / fa7e5a4, so HEAD is what the site is
  serving.  The upload directory is rebuilt each round to hold EXACTLY the
  files whose bytes differ from HEAD (or that HEAD does not have).  In round 1
  that was all 36 outputs; in round 2 the 20 SVGs and topics.html fall out
  because they are unchanged, leaving the 28 pages plus topics-en.html and
  sitemap.xml.
* the hub shares the prefix (pel.html / pel-en.html), so no _hub_name
  override -- see pel.py.
* the pel sitemap block goes in right after the brt block's last line
  (brt-after-en.html), before carc.html.
* disease-topic house priorities: 0.85/0.75 hubs, 0.75/0.65 articles.
* the topics card goes after the brt card (the last one on the pulled
  topics.html, which carries 9 cards).

This builder writes ONLY the pel- article pages and the pel hub, appends one
...
os.environ["PEL_STAGE"] = pel._STAGE
import stage_pel_figs

REPO = "/home/claude/repo"
UPLOAD = "/home/claude/upload-pel"
FIGS = "/home/claude/pel/figs"
BASE_REF = "HEAD"          # nothing pel- is published yet
...
LANGS = ("zh",) if pel.ZH_ONLY else ("zh", "en")

# The pel sitemap block is inserted as one run after the brt block's last line.
SM_ANCHOR = "brt-after-en.html"
...
def patch_sitemap(path, topic):
    ...
    block = [sitemap_entry("pel.html", "0.85", topic.DATE)]
    if "en" in LANGS:
        block.append(sitemap_entry("pel-en.html", "0.75", topic.DATE))
    block += [sitemap_entry("pel-%s.html" % s, "0.75", topic.DATE)
              for s in order]
    if "en" in LANGS:
        block += [sitemap_entry("pel-%s-en.html" % s, "0.65", topic.DATE)
                  for s in order]
    ...
    # round 2 appends its new URLs after the last pel line already present,
    # round 1 puts the whole block after the brt block
    anchor = SM_ANCHOR
    for name in reversed([re.search(re.escape(tb.BASE) + r"([^<]*)</loc>",
                          ...
                          if re.search(re.escape(tb.BASE) + r"([^<]*)</loc>",
                                       ln)]):
        if name == "pel.html" or name.startswith("pel-"):
            anchor = name
            break
...
def main():
    stage_pel_figs.STAGE = pel._STAGE
    stage_pel_figs.stage(zh_only=pel.ZH_ONLY)

    pages = []
    for lang in LANGS:
        pages += tb.build(pel, lang, ART_TPL[lang], REPO, zhonly=pel.ZH_ONLY)
        pages.append(tb.build_index(pel, lang, HUB_TPL[lang], REPO,
                                    zhonly=pel.ZH_ONLY))
    ...
    targets = [(os.path.join(REPO, "topics.html"),
                pel.TOPIC_CARD_ZH, "brt.html", pel.HASPART_ZH)]
    if "en" in LANGS:
        targets.append((os.path.join(REPO, "topics-en.html"),
                        pel.TOPIC_CARD_EN, "brt-en.html", pel.HASPART_EN))
    ...
    print("mode                : %s" % ("ZH-ONLY (round 1)" if pel.ZH_ONLY
                                        else "zh + en"))
