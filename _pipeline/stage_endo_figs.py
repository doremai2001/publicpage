# -*- coding: utf-8 -*-
"""Stage the em fragments for topicbuild: strip the em- prefix and insert the
site's article-figure markup at the position the manifest prescribes.

Unlike ec / gb, the English fragments under /home/claude/endo/en carry NO
-en suffix -- they are en/em-<slug>.html, the same basename as the Chinese
one under body/.  Only the directory tells them apart.

Like lv / brt / pel / ec / gb, every em figure carries placement.after_h4 --
the heading TEXT of the section the figure closes -- rather than a fixed
ordinal.  The heading is matched in the zh fragment and the same section
index is reused for the en fragment (h4 counts are asserted equal first).

PLACEMENT IS THE AUTHORITY.  All eleven em figures have a single-article
used_by that agrees with placement, so eleven figures make eleven insertions,
one per article in A1 / A2 / A3 / A4 / B1 / B2 / B4 / B5 / C1 / D2 / D3.

Figures are inserted HERE, into the staged copy the builder reads, so the
source fragments under /home/claude/endo/body and /home/claude/endo/en stay
...
SVG's viewBox height; the em SVGs carry integer heights, but the pel-style
round-half-up is kept so a fractional one would not break the build.

Set EM_STAGE to match endo.py's _STAGE.
"""
import decimal
import glob
import json
...
EM = "/home/claude/endo"
STAGE = os.environ.get("EM_STAGE", "./staging-em")
...
def svg_height(name):
    ...
    s = open(EM + "/figs/" + name, encoding="utf-8").read()
...
def insert_after_section(text, n, block):
    ...
    <h4> #(n+1).  Every em placement has a following section (asserted)."""
...
def stage():
    ...
    manifest = json.load(open(EM + "/figs/manifest.json", encoding="utf-8"))
    per = {}
    for fig in manifest:
        art = fig["placement"]["article"]
        assert art in fig["used_by"], fig["id"]
        per.setdefault(art, []).append(fig)
    for p in sorted(glob.glob(EM + "/body/*.html")):
        slug = os.path.basename(p)[:-5]              # em-<tail>
        zh = open(p, encoding="utf-8").read()
        en = open("%s/en/%s.html" % (EM, slug), encoding="utf-8").read()
