# -*- coding: utf-8 -*-
"""Verification pass over the generated liver-cancer pages (the cervix
12-check standard, plus the lv-specific disclosure-paragraph check 3c and the
manifest-driven figure placement in 3b)."""
import difflib, filecmp, html, json, os, re, sys
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
import liver, topicbuild as tb
...
ORD = [s for sec in liver.SECTIONS for s in sec["slugs"]]
...
def src_path(slug, lang):
    return os.path.join(LV, "body" if lang == "zh" else "en",
                        "lv-%s%s.html" % (slug, "" if lang == "zh" else "-en"))

hubs = ["liver.html", "liver-en.html"]
arts = ["lv-%s.html" % s for s in ORD] + ["lv-%s-en.html" % s for s in ORD]
pages = hubs + arts
svgs = sorted(n for n in os.listdir(os.path.join(LV, "figs")) if n.endswith(".svg"))

# 1 --------------------------------------- page count + sitemap, both ways ---
chk(len(pages) == 42, "page count %d" % len(pages))
for p in pages:
...
chk(len(os.listdir(UPLOAD)) == 65, "upload dir has %d files" % len(os.listdir(UPLOAD)))
sm = rd(REPO, "sitemap.xml")
pre_sm = rd(PRE, "sitemap.xml")
chk(sm.count("<url>") == pre_sm.count("<url>") + 42,
    "sitemap count %d (was %d)" % (sm.count("<url>"), pre_sm.count("<url>")))
for p in pages:
    chk(("<loc>%s%s</loc>" % (tb.BASE, p)) in sm, "sitemap missing %s" % p)
for p, prio in ([("liver.html", "0.85"), ("liver-en.html", "0.75")]
                + [("lv-%s.html" % s, "0.75") for s in ORD]
                + [("lv-%s-en.html" % s, "0.65") for s in ORD]):
    ln = [l for l in sm.splitlines() if "<loc>%s%s</loc>" % (tb.BASE, p) in l][0]
    chk("<lastmod>%s</lastmod>" % liver.DATE in ln, "%s lastmod" % p)
    chk("<priority>%s</priority>" % prio in ln, "%s priority" % p)
# existing entries and their order untouched
RE_LV_LOC = re.compile(r"<loc>%s(lv-|liver(-en)?\.html)" % re.escape(tb.BASE))
kept = [l for l in sm.splitlines() if not RE_LV_LOC.search(l)]
chk(kept == pre_sm.splitlines(), "existing sitemap lines changed or reordered")
chk("lv-lv-" not in sm and not any("lv-lv-" in n for n in os.listdir(REPO)),
    "double-prefixed name somewhere")
# the lv block sits right after the cx block, before bc.html
sml = sm.splitlines()
i_cx = [i for i, l in enumerate(sml)
        if "<loc>%scx-recurrence-en.html</loc>" % tb.BASE in l][0]
chk("<loc>%sliver.html</loc>" % tb.BASE in sml[i_cx + 1], "lv block not after cx block")
chk("<loc>%sbc.html</loc>" % tb.BASE in sml[i_cx + 43], "lv block not before bc block")
...
pre_not_sm = sorted(n for n in repo_html
                    if not (n.startswith("lv-") or n in hubs)
...
chk(all(p in sm_names for p in pages), "an lv page missing from sitemap")
print("1. 42 pages (40 articles + 2 hubs) exist in repo and upload dir; upload dir = "
      "%d files (42 pages + %d svgs + topics x2 + sitemap); sitemap <url> = %d "
      "(was %d, +42), priorities 0.85/0.75 hub and 0.75/0.65 articles, lastmod %s; "
      "lv block sits between the cx and bc blocks; all pre-existing lines "
      ...
      % (len(os.listdir(UPLOAD)), len(svgs), sm.count("<url>"),
         pre_sm.count("<url>"), liver.DATE, in_repo_not_sm or "none"))
...
for slug in ORD:
    ...
    for lang in ("zh", "en"):
        ...
        page = rd(REPO, "lv-%s%s.html" % (slug, "-en" if lang == "en" else ""))
...
manifest = json.load(open(os.path.join(LV, "figs", "manifest.json"), encoding="utf-8"))
per = {}
for fig in manifest:
...
for slug in ORD:
    ...
    for lang, suf in (("zh", ""), ("en", "-en")):
        s = rd(REPO, "lv-%s%s.html" % (slug, suf))
        figs = re.findall(
        ...
        chk(len(figs) == len(want), "lv-%s%s: %d figures, want %d"
        ...
        for got, fig in zip(figs, want):
            ...
            # (the lv-parallel caption legitimately contains "Newhauser & Zhang")
            cap = (fig["zh_caption"] if lang == "zh" else fig["en_caption"]
                   ).replace("&", "&amp;").replace("<", "&lt;")
            vb = re.search(r'viewBox="0 0 1440 (\d+)"', rd(LV, "figs", dsk)).group(1)
            chk(got[0] == mob, "lv-%s%s srcset %s" % (slug, suf, got[0]))
            chk(got[1] == dsk, "lv-%s%s src %s" % (slug, suf, got[1]))
            chk(got[2] == vb, "lv-%s%s height %s != viewBox %s" % (slug, suf, got[2], vb))
            chk(got[3] == alt, "lv-%s%s alt differs" % (slug, suf))
            chk(got[4] == cap, "lv-%s%s caption differs" % (slug, suf))
            nfig_checked += 1
        # placement audit in BOTH languages: figure #k closes the h4 section
        # the manifest names (matched by index against the zh source)
        ...
        for m in re.finditer(r'<h4>|<figure class="article-figure">', body):
            if m.group(0) == "<h4>":
                ...
                pos.append(h4)
        chk(pos == want_pos, "lv-%s%s figure placement %s want %s"
            % (slug, suf, pos, want_pos))
print("3b. %d figure instances match the manifest (files per language, alt "
      "attribute-escaped, caption, height = desktop viewBox); placement per "
      "manifest after_h4, audited in both languages (lv-three-roads carries "
      "two figures)" % nfig_checked)
for n in svgs:
    chk(filecmp.cmp(os.path.join(LV, "figs", n), os.path.join(REPO, n),
                    shallow=False), "svg differs in repo: %s" % n)

# 3c ------------------- disclosure paragraph, verbatim on all 20, per SPEC ---
for lang, suf in (("zh", ""), ("en", "-en")):
    src = rd(src_path("no-biopsy", lang))
    ps = re.findall(r"<p>.*?</p>", src[:src.index("<h4>")], re.S)
    disc = ps[1]                       # the fixed disclosure, 2nd lead-in <p>
    chk(("先說我的位置" in disc) if lang == "zh" else ("My position first" in disc),
        "%s disclosure paragraph not where expected" % lang)
    for slug in ORD:
        page = rd(REPO, "lv-%s%s.html" % (slug, suf))
        body = re.search(r'<div class="body-html">\n(.*?)\n    </div>',
                         page, re.S).group(1)
        head = body[:body.index("<h4>")]
        chk(head.count(disc) == 1,
            "lv-%s%s: disclosure paragraph missing or altered before first h4"
            % (slug, suf))
print("3c. the conflict-of-interest disclosure paragraph appears byte-identical "
      "before the first <h4> on all 20 articles, both languages")
...
# The cc template carries the figure css itself (the site owner added it),
...
for p in pages:
    ...
    seen.add(st)
chk(len(seen) == 1, "style not byte-stable across the 42")
chk(BLOCK.startswith("/* ---------- article figures ---------- */")
    and BLOCK.count(".article-figure") == 9, "figure css block looks wrong")
print("4. <style> on all 42 pages is byte-identical (%d bytes) and equals the shared "
      "template style (%d bytes) with the article-figure block present (inject_css "
      "idempotent: the cc template already carries it); head/body tags exactly once "
      "each" % (len(WANT_STYLE), len(tpl_style)))

# 5 ------------------------------------------------------------- hreflang ----
npair = 0
for slug in ORD + [None]:
    z = "liver.html" if slug is None else "lv-%s.html" % slug
    e = "liver-en.html" if slug is None else "lv-%s-en.html" % slug
    npair += 1
...
print("5. hreflang zh-Hant/en/x-default + canonical + nav language switch, both "
      "directions, on all %d pairs (20 articles + the hub)" % npair)

# 6 ------------------------------------------------- topics.html diff --------
for f, card, hp in (("topics.html", liver.TOPIC_CARD_ZH, liver.HASPART_ZH),
                    ("topics-en.html", liver.TOPIC_CARD_EN, liver.HASPART_EN)):
    ...
    chk(cards[-1] == ("liver.html" if f == "topics.html" else "liver-en.html")
        and cards[-2].startswith("cx"), "%s card order %s" % (f, cards))
    ...
    chk(len(ld["hasPart"]) == 8, "%s hasPart %d" % (f, len(ld["hasPart"])))
    print("6. %-15s differs from the pulled version ONLY by the lv card (after the "
          "cervical card, %d lines) and the hasPart entry; 8 cards in the 2-column "
          "grid; hasPart = 8 hubs ending with liver" % (f, len(rest)))

# 7 ------------------------------------------------------------- hub render ---
for lang, page in (("zh", "liver.html"), ("en", "liver-en.html")):
    s = rd(REPO, page)
    ...
    chk(all(g.count('class="postcard"') == 5 for g in grp), "%s group sizes" % page)
    for i, slug in enumerate(ORD):
        chk(cards[i][1] == "lv-%s%s.html" % (slug, "-en" if lang == "en" else ""),
    ...
    ld = json.loads(re.search(r'<script type="application/ld\+json">(.*?)</script>',
                              s, re.S).group(1))
    chk(len(ld["hasPart"]) == 20, "%s hasPart %d" % (page, len(ld["hasPart"])))
...
print("7. hub: 4 groups x 5 cards in reading order; 全部/All button = 20; "
      "%d per-tag counts each equal the number of cards carrying that tag; "
      "JSON-LD hasPart lists 20; new labels sbrt/liverfunction/transplant/"
      "tace/hepatitis rendered as agreed in both languages" % (len(want) - 1))
...
for p in pages:
    ...
    chk(ld["about"]["name"] == ("Liver cancer" if en else "肝癌"),
...
print("8. nav active state per language; JSON-LD about = MedicalCondition "
      "(肝癌 / Liver cancer) on all 42; datePublished = dateModified = "
      "2026-08-30 on all 40 articles")
...
for lang, suf in (("zh", ""), ("en", "-en")):
    for i, slug in enumerate(ORD):
        ...
        if i == 0:
            ...
            chk('class="pv" href="lv-%s%s.html"' % (ORD[i - 1], suf) in pn,
                "%s%s prev" % (slug, suf))
        if i == len(ORD) - 1:
            chk(pn.endswith("<span></span>"), "%s%s last has next" % (slug, suf))
        else:
            chk('class="nx" href="lv-%s%s.html"' % (ORD[i + 1], suf) in pn,
                "%s%s next" % (slug, suf))
print("9. pnav chains all 20 in reading order, both languages; ends use <span></span>")
...
for lang, suf in (("zh", ""), ("en", "-en")):
    ...
    for slug in ORD:
        g = skeleton(rd(REPO, "lv-%s%s.html" % (slug, suf)))
        chk(g == t, "lv-%s%s: skeleton differs from %s" % (slug, suf, ART_TPL[lang]))
for lang, page in (("zh", "liver.html"), ("en", "liver-en.html")):
    t = skeleton(rd(REPO, HUB_TPL[lang]))
    g = skeleton(rd(REPO, page))
    chk(g == t, "%s: skeleton differs from %s" % (page, HUB_TPL[lang]))
print("10. every byte outside the substituted slots (and the style, checked in 4) "
