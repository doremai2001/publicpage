# -*- coding: utf-8 -*-
"""Build the glioblastoma topic into the working clone, patch the two topic
index pages (card + JSON-LD hasPart) and the sitemap, copy the figure SVGs
in, mirror everything new/changed into the upload directory, and report the
counts plus an internal-link scan.

Same driver as build_liver.py / build_esoph.py.  Differences for gb:
...
* before the first patch, the pulled topics.html / topics-en.html /
  sitemap.xml are snapshotted to /home/claude/gbm/prebackup, which the
  verifier diffs against;
* the hub is gbm.html / gbm-en.html while the articles are gb-*, so gbm.py
  overrides topicbuild._hub_name at import time (the liver shape);
* the gb sitemap block goes in right after the ec block's last line
  (ec-recurrence-en.html), before carc.html;
* disease-topic house priorities: 0.85/0.75 hubs, 0.75/0.65 articles;
* the topics pages' card goes after the ec card (the last one on the pulled
  topics pages, which carry 11 cards).

This builder writes ONLY the gb- article pages and the gbm hubs, appends one
card per topics page and one sitemap block -- no other topic's pages are ever
...
os.environ["GB_STAGE"] = gbm._STAGE
import stage_gbm_figs

REPO = "/home/claude/repo"
UPLOAD = "/home/claude/upload-gbm"
FIGS = "/home/claude/gbm/figs"
...
# The gb sitemap block is inserted as one run after the ec block's last line.
SM_ANCHOR = "ec-recurrence-en.html"
CARD_ANCHOR = {"zh": "ec.html", "en": "ec-en.html"}
...
def patch_sitemap(path, topic):
    ...
    if any(loc("gbm.html") in ln for ln in lines):
        return False

    block = ([sitemap_entry("gbm.html", "0.85", topic.DATE),
              sitemap_entry("gbm-en.html", "0.75", topic.DATE)]
             + [sitemap_entry("gb-%s.html" % s, "0.75", topic.DATE)
                for s in order]
             + [sitemap_entry("gb-%s-en.html" % s, "0.65", topic.DATE)
                for s in order])
...
def main():
    ...
    stage_gbm_figs.STAGE = gbm._STAGE
    stage_gbm_figs.stage()

    pages = []
    for lang in ("zh", "en"):
        pages += tb.build(gbm, lang, ART_TPL[lang], REPO)
        pages.append(tb.build_index(gbm, lang, HUB_TPL[lang], REPO))
    ...
    for path, card, anchor, hp in (
        (os.path.join(REPO, "topics.html"),
         gbm.TOPIC_CARD_ZH, CARD_ANCHOR["zh"], gbm.HASPART_ZH),
        (os.path.join(REPO, "topics-en.html"),
         gbm.TOPIC_CARD_EN, CARD_ANCHOR["en"], gbm.HASPART_EN),
    ):
        if patch_topics(path, card, anchor, hp):
            changed.append(path)

    smap = os.path.join(REPO, "sitemap.xml")
    if patch_sitemap(smap, gbm):
        changed.append(smap)
